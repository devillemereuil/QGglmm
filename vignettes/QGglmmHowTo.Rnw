...
\usepackage{float}		%Option des figures et tables (floats)
\usepackage[footnotesize]{caption}	%Option des légendes
...
\begin{wrapfigure}[28]{l}{0.3\textwidth}
 \centering
 \includegraphics[width=0.3\textwidth]{Figs/3scales.pdf}
...
\begin{wrapfigure}[6]{l}{0.4\textwidth}
 \centering
 \vspace{-0.8cm}
 \includegraphics[width=0.4\textwidth]{Figs/latent.pdf}
...
\begin{Schunk}
 \begin{Sinput}
mu=0     #Value for the intercept mu
Va=0.3   #Value for the additive genetic variance
Vo=0.3   #Value for the over-dispersion variance
...
\begin{wrapfigure}[9]{r}{0.4\textwidth}
 \centering 
 \vspace{-0.8cm}
 \includegraphics[width=0.4\textwidth]{Figs/expected.pdf}
...
\begin{wrapfigure}[6]{r}{0.4\textwidth}
 \centering
 \vspace{-1cm}
 \includegraphics[width=0.4\textwidth]{Figs/observed.pdf}
...
\begin{Schunk}
 \begin{Sinput}
model <- MCMCglmm(phen~1,random=~animal,pedigree=pedigree,
...
\begin{Schunk}
 \begin{Sinput}
model <- MCMCglmm(cbind(phen1,phen2)~trait-1,random=~us(trait):animal,
	 rcov=~us(trait):units,family=c("ordinal","gaussian"),prior=prior,data=data)
 \end{Sinput}
\end{Schunk}
...
\begin{Schunk}
 \begin{Sinput}
mu <-  c(mean(model[["Sol"]][,"traitphen1"]),mean(model[["Sol"]][,"traitphen2"]))
G <- matrix(c(mean(model[["VCV"]][,"traitphen1:traitphen1.animal"]),
     mean(model[["VCV"]][,"traitphen1:traitphen2.animal"]),
     mean(model[["VCV"]][,"traitphen1:traitphen2.animal"]),
     mean(model[["VCV"]][,"traitphen2:traitphen2.animal"])),ncol=2)
R <- matrix(c(mean(model[["VCV"]][,"traitphen1:traitphen1.units"]),
     mean(model[["VCV"]][,"traitphen1:traitphen2.units"]),
     mean(model[["VCV"]][,"traitphen1:traitphen2.units"]),
     mean(model[["VCV"]][,"traitphen2:traitphen2.units"])),ncol=2)
P <- G + R
 \end{Sinput}
\end{Schunk}
...
\begin{Schunk}
 \begin{Sinput}
QGparams(mu=mu,var.a=va,var.p=vp,model="Poisson.log",closed.form=FALSE)
 \end{Sinput}
 \begin{Soutput}
...
\begin{Schunk}
 \begin{Sinput}
  var.func <- function(x)\{(exp(x)/(1+exp(x))) * (1 - (exp(x)/(1+exp(x))))\}
 \end{Sinput}
\end{Schunk}
QGglmm requires that these functions are provided using a named list:
\begin{Schunk}
 \begin{Sinput}
custom.functions <- list(inv.link=inv.link,var.func=var.func,d.inv.link=d.inv.link)
 \end{Sinput}
\end{Schunk}
Now, we can use our ``custom'' model in QGparams:
\begin{Schunk}
 \begin{Sinput}
QGparams(mu=0.5,var.a=0.5,var.p=1,custom.model=custom.functions)
 \end{Sinput}
 \begin{Soutput}
[1] "Computing observed mean..."
[1] "Computing variances..."
...
\begin{Schunk}
 \begin{Sinput}
QGmvparams(mu=mu,vcv.G=G,vcv.P=P,model=c("binom1.probit","Gaussian","Poisson.log"))
 \end{Sinput}
 \begin{Soutput}
[1] "Computing observed mean..."
...
This is much more complex than univariate models and there are so many possibilities that using closed forms for the multivariate is practically impossible. Hence \texttt{QGmvparams} \textit{always} resort to integral computations (using the \href{https://cran.r-project.org/web/packages/R2Cuba/index.html}{R2Cuba} package).\\
You will probably also have noted that the computation was substantially slower than in the univariate case!
...
\begin{Schunk}
 \begin{Sinput}
QGpred(mu=mu,var.a=va,var.p=vp,fit.func=fit,d.fit.func=d.fit)
 \end{Sinput}
 \begin{Soutput}
...
\begin{Schunk}
 \begin{Sinput}
delta.mu <- QGpred(mu=mu,var.a=va,var.p=vp,fit.func=fit,d.fit.func=d.fit)\$lat.resp
QGmean(mu=mu, var=vp,link.inv=QGlink.funcs("binom1.probit")\$inv.link)
QGmean(mu=mu+delta.mu, var=vp,link.inv=QGlink.funcs("binom1.probit")\$inv.link)
 \end{Sinput}
 \begin{Soutput}
0.3817207
0.3929478
...
\begin{Schunk}
 \begin{Sinput}
model <- MCMCglmm(phen~nutr,random=~animal,pedigree=pedigree,
	 data=data,prior=prior,family="ordinal")
 \end{Sinput}
\end{Schunk}
...
va <- mean(model[["VCV"]][,"animal"])
vp <- mean(rowSums(model[["VCV"]]))
QGparams(predict=yhat,var.a=va, var.p=vp,model="binom1.probit")
 \end{Sinput}
 \begin{Soutput}
...
\begin{Schunk}
 \begin{Sinput}
QGicc(mu=mu,var.comp=vi,var.p=vp,model="Poisson.log")
 \end{Sinput}
 \begin{Soutput}
[1] "Using the closed forms for a Poisson-log model."
  mean.obs var.obs var.comp.obs   icc.obs
1 4.481689 38.9943     13.02992 0.3341492
 \end{Soutput}
\end{Schunk}
...
Basically, \texttt{QGicc} has all the features \texttt{QGparams} has: fixed-effects can be accounted for using the \texttt{predict} function and it has a multivariate counter-part \texttt{QGmvicc} which yields variance-covariance matrix on the observed data scale. Note that this multivariate function, like \texttt{QGmvparams} never uses closed forms. As a consequence, because of the double integration, it can be quite slow and painfully slow when fixed effects are included!\\
...
df <- data.frame(mu=as.vector(model[["Sol"]][,"(Intercept)"]),
      va=as.vector(model[["VCV"]][,"animal"]),vp=rowSums(model[["VCV"]]))
head(df)
 \end{Sinput}
 \begin{Soutput}
         mu       va       vp
...
df <- cbind(mu1=model[["Sol"]][,"traitphen1"],
      mu2=model[["Sol"]][,"traitphen2"],model[["VCV"]])
 \end{Sinput}
\end{Schunk}
Now, we need to do is construct the matrix ``on the fly'' before calling \texttt{QGmvparams}:
\begin{Schunk}
 \begin{Sinput}
...
g <- c(-Inf,cp,Inf)	#All cut-points including the infinite extremes
 \end{Sinput}
\end{Schunk}
...
\begin{Schunk}
 \begin{Sinput}
z<-apply(p,2,function(vec){vec<-as.vector(vec);rmultinom(1,size=1,prob=vec)})
z
 \end{Sinput}
 \begin{Soutput}
...
\begin{Schunk}
 \begin{Sinput}
out <- QGparams(mu=mu,var.a=va,var.p=vp,model="ordinal",cut.points=c(-Inf,cp,Inf))
out
 \end{Sinput}
 \begin{Soutput}
\$mean.obs
...
\begin{Schunk}
 \begin{Sinput}
eigen(cov2cor(out[["vcv.G.obs"]]))\$values	#Only 1 non-null eigen value
 \end{Sinput}
 \begin{Soutput}
